...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='data_firev6/yolov3-tiny.cfg', help='cfg file path')
    parser.add_argument('--data', type=str, default='data_firev6/yolo.data', help='*.data file path')
    parser.add_argument('--weights', type=str, default='weights/last.pt', help='sparse model weights')
    parser.add_argument('--percent', type=float, default=0.5, help='global channel prune percent')
    parser.add_argument('--img_size', type=int, default=416, help='inference size (pixels)')
    parser.add_argument('--truncated_layer', type=int, default=84, help='the first truncated layer')
    ...
    print(f'Highest Threshold should be less than {highest_thre:.4f}.')
    print(f'The  corresponding prune ratio  percent_limit is {percent_limit:.3f}.')
    # 获得在目标百分百比下的剪植阈值
   ...
   # print('(mp, mr, map, mf1, *(loss.cpu() / len(dataloader)).tolist()), maps'.format(pruned_model_metric))
    print("\n the following  prunedis just set the values under the threshold  to be zero,\n"
          " this is not real prune, but give  us a hint ,\n"
          "so that if pruned result is not to low   which we can use fine-tune or distilling  to take the accuracy back \n")
    print('after pruned_model_but_keep_size map is {}'.format(mAP))
    ...
    print("\n After  pruned values, pruned model the number of parameters:\n")
    compact_nparameters = obtain_num_parameters(compact_model)
    # 拷贝权重
    init_weights_from_loose_model(compact_model, pruned_model, CBL_idx, Other_idx, CBLidx2mask)
    # 测试运行速度
    random_input = torch.rand((4, 3, 416, 416)).to(device)
    ...
    # 生成剪枝后的cfg文件并保存模型
    pruned_cfg_name = opt.cfg.replace('/', f'/analysis_normal_prune_{percent}_')
    # 创建存储目录
    ...
    compact_model_name = opt.weights.replace( opt.weights.split('/')[-1], f'analysis_normal_prune_{str(percent)}_percent.weights')

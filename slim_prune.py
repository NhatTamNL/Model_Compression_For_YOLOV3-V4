...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='weights/v4_tiny_416.pt', help='sparse model weights')
    ...
    CBL_idx, Conv_idx, prune_idx, _, _ = shortcut_parse_module_defs(model.module_defs)
    ...
    and_merge_mask(model, CBLidx2mask, CBLidx2filters)
    ...
    pruned_cfg_name = opt.cfg.replace('/', f'/slim_prune_{opt.percent}_keep_{opt.layer_keep}_')
    # 创建存储目录
    ...
    compact_model_name = opt.weights.replace(opt.weights.split('/')[-1], f'slim_prune_{opt.percent}_layer_keep_{opt.layer_keep}.weights')

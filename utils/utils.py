...
from  utils.torch_utils import * # , google_utils
...
def compute_lost_group_AT_KD(model, targets, output_s, output_t, feature_s, feature_t, batch_size):
    ...
    Lambda_cls, Lambda_box, Lambda_feature = 0.001, 0.001, 0.008
    criterion_st = torch.nn.KLDivLoss(reduction='sum')
...
def compute_lost_group_AT_KD_for_v3_darknet53(model, targets, output_s, output_t, feature_s, feature_t, batch_size):
    ...
    all_pair_layer_attention_loss = 0
    # attention layers
    at_layer = [2,5,11,24,30,40,45,56, 59,61, 67, 70]
...
def compute_lost_group_AT_KD_for_v3_mobilenet(model, targets, output_s, output_t, feature_s, feature_t, batch_size):
    ...
    all_pair_layer_attention_loss = 0
    # attention layers
    at_layer = [3,4,6,8,15,20,30,40,50,60, 63,67, 70, 73 ]

    group1 = [3,4,6,8,15,20]
    group2 = [30, 40, 50, 60]
    group3 = [63, 67, 70, 73]
...
def compute_lost_fine_grained_group_AT_KD(model, targets, output_s, output_t, feature_s, feature_t,batch_size, img_size):
    ...
    for i in  range(len(at_layer)):
        ...
        single_feature_s = feature_s[at_layer[i]]
        ...
        fine_grained_t = (single_feature_t * mask).view(single_feature_t.size(0),-1)
        fine_grained_s = (single_feature_s * mask).view(single_feature_s.size(0),-1)
...
def strip_optimizer(f='weights/v4_tiny_416_map61.pt'):  # from utils.utils import *; strip_optimizer()
...
def create_backbone(f='weights/v4_tiny_416_map61.pt'):  # from utils.utils import *; create_backbone()
